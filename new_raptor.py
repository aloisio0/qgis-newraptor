...
class NewRaptor:
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.cmbSpecies.currentTextChanged.connect(
                self.evt_cmbSpecies_changed)  # Conectando a função ao cmb Species

        mc = self.iface.mapCanvas()  # Referência a tela do mapa
        self.dlg.spbLatitude.setValue(mc.center().y())  # Obtendo a latutide do centro do mapa
        self.dlg.spbLongitude.setValue(mc.center().x())  # Obtendo a longitude do centro do mapa
        self.dlg.dteLast.setDate(QDate.currentDate())  # Obtendo a data atual

        map_layers = []  # Lista que conterá os nomes dos layers disponíveis no índice do QGIS
        ...
        if result:
            ...
            valNestID = lyrNests.maximumValue(
                idxNestID) + 1  # Determinando o índice do Feature que será adicionado com base no índice do último Feature na camada "Raptor Buffer"
            ...
            QMessageBox.information(self.dlg, "Message", f"ID: {valNestID}\nLatitude: {valLat}\nLongitude: {valLng}"
                                                         f"\nSpecies: {valSpecies}\nBuffer: {valBuffer}"
                                                         f"\nStatus: {valStatus}\nDate: {valLast}")

            ftrNest = QgsFeature(lyrNests.fields())  # Acessando os campos (fields) do layer Nests
            #  Adicionando os atributos do Novo Feature
            ...
            geom = QgsGeometry(QgsPoint(valLng, valLat))  # Criando uma gemetria/Ponto
            ftrNest.setGeometry(geom)  # Definindo a geometria

            pr = lyrNests.dataProvider()  # Acessando o provedor de dados
            pr.addFeatures([ftrNest])  # Adicionando o Feature no Layer
            lyrNests.reload()  # Atualizar o Layer

            pr = lyrBuffer.dataProvider()  # Acessando o provedor de dados
            buffer = geom.buffer(valBuffer, 10)  # Definindo a geometria do Buffer
    ...
    def evt_cmbSpecies_changed(self, species):
        """Vai mudar o valor do Buffer dependendo de qual Species for selecionada"""
